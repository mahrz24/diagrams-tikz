...
-----------------------------------------------------------------------------
-- |
-- Module      :  Diagrams.Backend.TikZ.CmdLine
-- Copyright   :  (c) 2011 Diagrams team (see LICENSE)
-- License     :  BSD-style (see LICENSE)
-- Maintainer  :  diagrams-discuss@googlegroups.com
--
-- Convenient creation of command-line-driven executables for
-- rendering diagrams using the TikZ backend.
--
-----------------------------------------------------------------------------
...
defaultMain :: Diagram TikZ R2 -> IO ()
defaultMain d = do
  prog <- getProgName
  args <- getArgs
...
chooseRender :: DiagramOpts -> Diagram TikZ R2 -> IO ()
chooseRender opts d =
  case splitOn "." (output opts) of
    [""] -> putStrLn "No output file given."
    ps | last ps `elem` ["tex"] -> do
           let sizeSpec = case (width opts, height opts) of
                            ...
                            (Just w, Just h)   -> Dims (fromIntegral w)
                                                       (fromIntegral h)
           renderDia TikZ (TikZOptions (output opts) sizeSpec LaTeX) d
       | otherwise -> putStrLn $ "Unknown file type: " ++ last ps

multiMain :: [(String, Diagram TikZ R2)] -> IO ()
multiMain ds = do
  prog <- getProgName
